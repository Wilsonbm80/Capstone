...
int reverse_layer[5][TOTAL_CHORDS * BEATS_PER_MEASURE];
int temp_layer[5][TOTAL_CHORDS * BEATS_PER_MEASURE];
int transposefactor;
int tempo;
int scale_array[9 * 8];
...
void make_reverse_layers(void)
 {
  ...
  for (i = 0; i <  TOTAL_CHORDS * BEATS_PER_MEASURE; ++i)
    for (j = 0; j < 5; ++j)
        if (layer[j][i] >= 0 && layer[j][i] <= 127)
            reverse_layer[j][i] =  127 - layer[j][i];
         else if (layer[j][i] == -1) 
           reverse_layer[j][i] = layer[j][i];
         else if (layer[j][i] >= 128)
           {
             temp_layer[j][i] = layer[j][i] - 128; //gives note before adding sustaining 128
             reverse_layer[j][i] = 127 - temp_layer[j][i]; //gives reverse note before adding sustaining 128
             reverse_layer[j][i] = reverse_layer[j][i] + 128; // gives reverse note sustaining 
           }
  for (i = 0; i < TOTAL_CHORDS * BEATS_PER_MEASURE; ++i)
    for (j = 0; j < 5; ++j)
        layer[j][i] = reverse_layer[j][i];
...
/* now make first measure of phrase the TONIC chord */
  ...
  chords_a[0].quality = MAJOR;
  sound[0] = BEGINNING_PITCH + transposefactor;
  makechords (MAJOR, sound);
  for (j = 0; j < 3; ++j)
...
void
make_arpeggiated_bass (int pattern[], struct sounds chords[], int arpeggio[])
{
...
void make_scale_array (void)
{
  ...
  for (i = 0; i < ((9 * 8) - 2); ++i)
    {
      scale_array[i + 1] = scale_array[i] + ionian[mode_marker];
      ++mode_marker;
      if (mode_marker == 7)
	mode_marker = 0;
    }
