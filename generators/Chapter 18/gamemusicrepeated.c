...
int rhythm[21][8] = {{1,1,2,1,1,2,0,0},     //0    may randomize order of these. Only 4th four will be main theme after randomization.  
      ...
      // 4 special 1 beat measures       //16
                     ...
                     {1,1,0,0,0,0,0,0},  //18   
                     {1,1,0,0,0,0,0,0},  //19
                     {2,0,0,0,0,0,0,0}}; //20  array is not [21][8] 
...
void length_first_intensity_arp(void)
 {
...
int choose_chords_pitch(void)
  {
   ...
   x = rand()%3;
   if (x == 0) y = sounds[0];
   else if (x == 1) y = sounds[2]; 
   else if (x == 2) y = sounds[4];
   return(y);
  }
