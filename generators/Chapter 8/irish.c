...
#define MAJOR 0
#define MINOR 1
#define OPEN 2			/* THIS IS NEW */
#define THIRD 3			/* This IS NEW */
#define LOWER_SUS 4
#define UPPER_SUS 5
#define FIFTH 6
...
/* 128 = 4 measures times  16 phrases times 2 */
/*for eighth notes  */
...
void make_arpeggiated_bass (int pattern[], struct sounds chords[],
			    int arpeggio[]);
...
void make_scale_array (void)
{
  int pentatonic[] = { 2, 2, 3, 2, 3 };
  int mode_marker = 0;
  int i;
  scale_array[0] = TONIC + transposefactor;
  for (i = 0; i < ((9 * 8) - 2); ++i)
    {
      scale_array[i + 1] = scale_array[i] + pentatonic[mode_marker];
      ++mode_marker;
      if (mode_marker == 5)
	mode_marker = 0;
    }
...
/* -1 represents a silence. 128 + note means that note is tied */
  ...
  int volume;
  int coda_slowdown = 0;
  int channel[8] = { 144, 146, 146, 146, 145, 147, 147, 147 };
