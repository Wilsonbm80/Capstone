/* mirror1.c - produces mirror structures */
/* Here's how it works. First set up a fragment of an artificial scale. Call it 
the pattern. From the note it ends on continue the patterns by 
having mirror inversion of the intervals. The length of the fragment is 
the PATTERNLENGTH. The intervals that make up the fragment are the 
INTERVALS. Now start that the lowest frequency and continue up to the 
highest frequecy using the method. Each time the note comes to the end 
of the fragment and starts to reverse itself, that note is called a node. 
The composition works by  having a wandering melody. Each time it stumbles 
onto a node, this node is called the primary_node. Some of the other nodes 
are active_nodes. The distance from the melody to the current primary node 
is reflected in the other current active nodes produces other notes called 
generated_notes. The melody along with the generated_notes are what the 
listener will hear. Thus the melody will wander along, and will grow 
or shrink like a crystal when is hits different nodes. 
  */
#include<stdio.h>
#include<stdlib.h>
#include<time.h>
