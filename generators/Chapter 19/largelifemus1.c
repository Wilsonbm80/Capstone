...
#define LENGTH  20   // playing area is 18 by 18 with border on all 4 sides  
#define WIDTH   20  //  
...
int tempj;
int matrix[18][18] =
{ // in matrix[i][j], the j is from left to right in matrix as j increases, and i is top to bottom in matrix as i increases 
...
{102,103,104,105,106,107,108,109,110,111, 110,109,108,107,106,105,104,103}, 
...
*/
 ...
 for (j = 0; j < WIDTH;++j)
   ...
   if (board[i][j] == 'x' && board1[i][j] != board[i][j])
    ...
    if ( (i >=10 || i <=17) && (j >=0 || j <= 9) )
     play(0,145,matrix[i-1][j-1],60);
    if ( (i >=0 || i <=9) && (j >=10 || j <= 17) )
